...
def add_front_matter(doc: Document) -> None:
    ...
    abstract_paras = [
        "随着移动互联网和微信生态的快速发展，购物小程序已经成为中小型商家开展线上经营的重要载体。然而，现有不少购物类小程序仍存在功能分散、业务链路不完整、售前售后咨询效率较低以及后台运营支撑不足等问题，用户往往需要在商品浏览、下单确认、订单查询和客服咨询之间频繁切换，影响整体购物体验。针对上述问题，本文结合真实项目开发过程，设计并实现了一套面向移动端购物场景的智能购物小程序系统。",
        "本系统采用前后端分离架构，前端基于 uni-app、Vue 语法和 Pinia 状态管理实现用户端与管理端页面，后端基于 Node.js 与 Express 构建 RESTful API，数据库采用 MySQL 完成用户、商品、购物车、订单、售后、反馈及聊天记录等数据持久化。系统围绕商品展示、搜索筛选、购物车管理、订单流转、地址维护、售后申请和后台管理等核心业务进行实现，并引入 AI 智能客服模块。创新点在于：将商品上下文感知问答能力嵌入商品详情与聊天场景中，由后端拼接商品标题、规格、服务标签和发货时效等事实信息后调用大模型接口，同时结合调用日志记录和异常兜底机制提升回答稳定性。",
        "系统测试结果表明，该系统能够较好地完成移动端购物场景下的主要业务闭环，实现用户端与管理端的数据联动，提高商品咨询效率和后台运营管理便利性。本文的研究与实现验证了 uni-app、Node.js、MySQL 与 AI 能力在轻量级电商系统中的可行性，为校园商城、个体商户和中小型移动电商应用提供了一种具有参考价值的设计思路与实现方案。",
    ]
    for text in abstract_paras:
        add_para(doc, text)
    add_para(doc, "关键词：微信小程序；Node.js；MySQL；智能客服；电商系统")
    doc.add_page_break()
